...
import CooperativeInvoiceModal from "../../components/common/CooperativeInvoiceModal";
...
export default function CustomerDashboard() {
  ...
  return (
    <main className="bg-[var(--color-bg-elevated)] text-[var(--color-heading)] min-h-screen pt-24 pb-20 overflow-hidden relative">
                    ...
                    {quickBookings.length === 0 ? (
                      ...
                      quickBookings.map((qb) => (
                        <motion.div key={qb.id} variants={fadeUp} className="bg-[var(--color-surface)] border border-[var(--color-border)] p-6 rounded-2xl shadow-card relative overflow-hidden group">
                          ...
                          {/* Card Footer: Invoice & Review Options */}
                          <div className="mt-4 pt-4 border-t border-[var(--color-border)] flex items-center justify-between">
                            <button
                              onClick={() => setInvoiceModalId(qb.id)}
                              className="flex items-center gap-1 text-[11px] font-bold text-[#0B3C5D] bg-[#EDF4F9] hover:bg-[#D6E6F0] px-3 py-1.5 rounded-lg border border-[#D6E6F0] transition-colors"
                            >
                              <FiFileText size={13} />
                            ...
                            {qb.status === "COMPLETED" && !qb.rating && (
                              <button
                                ...
                                className="flex items-center gap-1.5 px-4 py-2 bg-[#0B3C5D] text-white font-bold text-[10px] uppercase tracking-wider rounded-lg hover:bg-[#082B42] transition-all active:scale-95 shadow-sm"
                              >
      ...
      {/* Official Cooperative Tax Invoice Modal */}
      <CooperativeInvoiceModal
        bookingId={invoiceModalId}
        isOpen={Boolean(invoiceModalId)}
